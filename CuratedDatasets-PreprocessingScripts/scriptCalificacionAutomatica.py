...
cwd = "./Calificacion Automatica"
raw_folder = "./raw_csv_data"

encabezadoString = "comment;CdificultadesImplementacionCparticipacionDocente;CdificultadesImplementacionCPocoUsoPlataforma;ClogrosPedagogicosCalificacionAutomaticaCaprendizajeAutonomo;ClogrosPedagogicosCalificacionAutomaticaCevaluacionOptimizada;CmejorableCalificacionAutomaticaCfallasGenerales;CmejorableCalificacionAutomaticaCFallasValidacion;CmejorableCalificacionAutomaticaCIncomprensible;CmejorableCalificacionAutomaticaCretroalimentacionInsuficiente;dificultadesImplementacion;facilidadUso;logrosPedagogicosCalificacionAutomatica;mejorableCalificacionAutomatica"
encabezado = ["CdificultadesImplementacionCparticipacionDocente","CdificultadesImplementacionCPocoUsoPlataforma","ClogrosPedagogicosCalificacionAutomaticaCaprendizajeAutonomo","ClogrosPedagogicosCalificacionAutomaticaCevaluacionOptimizada","CmejorableCalificacionAutomaticaCfallasGenerales","CmejorableCalificacionAutomaticaCFallasValidacion","CmejorableCalificacionAutomaticaCIncomprensible","CmejorableCalificacionAutomaticaCretroalimentacionInsuficiente","dificultadesImplementacion","facilidadUso","logrosPedagogicosCalificacionAutomatica","mejorableCalificacionAutomatica"]
...
f = open("calificacionAutomaticaGeneratedTexts.csv","a+") 
f.truncate(0)
...
f_calificacion = open("calificacionAutomaticaCalificacion.csv","a+") 
  ...
  if filename.endswith(".csv"): 
    ...
    for row in reader:
      if temp_counter == 0:
        temp_counter+=1
        continue
      comment = row[10].strip().replace("\n","").translate(transMap).strip()
      calificacion = row[9].strip().replace("\n","").translate(transMap).strip()
      #print(comment)
...
excel_file_path_calificacion = r'calificacionAutomaticaCalificacion.csv' #! toca dejar la r para que sea raw string y lea la url
...
excel_file_path = r'calificacionAutomaticaGeneratedTexts.csv' #! toca dejar la r para que sea raw string y lea la url
...
grouped_df.to_csv('calificacionAutomaticaDataFrameWithoutCalificacion.csv',sep=';',index=False,quoting=csv.QUOTE_NONE, quotechar="",escapechar="\\") #

# # read csv to dataframe
# df2 = pd.read_csv(r'calificacionAutomaticafinalDataFrame.csv',sep=';') 
# print(df2)
...
# created dataframe to csv with grouping for comments and categories
merged_df.to_csv('calificacionAutomaticaFinalDataFrameWithCalificacion.csv',sep=';',index=False,quoting=csv.QUOTE_NONE, quotechar="",escapechar="\\") #
