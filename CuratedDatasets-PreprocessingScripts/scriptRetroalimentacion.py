...
cwd = "./retroalimentacion"
raw_folder = "./raw_csv_data"

encabezadoString = "comment;CmejorableRetroalimentacionCfallasFuncionamiento;CmejorableRetroalimentacionCorientacionInsuficiente;condicionesIniciales;CresolucionProblemasCcomparacionConSalidasEsperadas;CresolucionProblemasCcorregirErrores;CresolucionProblemasCespecificacionesRetroalimentacion;mejorableRetroalimentacion;resolucionProblemas;senalarErrores"
encabezado = ["CmejorableRetroalimentacionCfallasFuncionamiento","CmejorableRetroalimentacionCorientacionInsuficiente","condicionesIniciales","CresolucionProblemasCcomparacionConSalidasEsperadas","CresolucionProblemasCcorregirErrores","CresolucionProblemasCespecificacionesRetroalimentacion","mejorableRetroalimentacion","resolucionProblemas","senalarErrores"]
...
f = open("retroalimentacionGeneratedTexts.csv","a+") 
f.truncate(0)
...
f_calificacion = open("retroalimentacionCalificacion.csv","a+") 
  ...
  if filename.endswith(".csv"): 
    ...
    for row in reader:
      if temp_counter == 0:
        temp_counter+=1
        continue
      comment = row[12].strip().replace("\n","").translate(transMap).strip()
      calificacion = row[11].strip().replace("\n","").translate(transMap).strip()
      #print(comment)
...
excel_file_path_calificacion = r'retroalimentacionCalificacion.csv' #! toca dejar la r para que sea raw string y lea la url
...
excel_file_path = r'retroalimentacionGeneratedTexts.csv' #! toca dejar la r para que sea raw string y lea la url
...
grouped_df.to_csv('retroalimentacionDataFrameWithoutCalificacion.csv',sep=';',index=False,quoting=csv.QUOTE_NONE, #quotechar="",
                  escapechar="\\") #

# # read csv to dataframe
# df2 = pd.read_csv(r'retroalimentacionfinalDataFrame.csv',sep=';') 
# print(df2)
...
# created dataframe to csv with grouping for comments and categories
merged_df.to_csv('retroalimentacionFinalDataFrameWithCalificacion.csv',sep=';',index=False,quoting=csv.QUOTE_NONE, quotechar="",escapechar="\\") #
